...
//    }
     ...
     *
     * @param keyValue   待插入关键字
     * @param index 待插入的位置
     */
    void insertKey(KeyValue keyValue, int index) {
        System.arraycopy(keyValues, index, keyValues, index + 1, keyNumber - index);
     ...
     * 3. 移除指定 index 处的 key
     */
    KeyValue removeKey(int index) {
        KeyValue t = keyValues[index];
        System.arraycopy(keyValues, index + 1, keyValues, index, --keyNumber - index);
        return t;
    }
    ...
    void moveToTarget(Node target) {
        ...
        for (int i = 0; i < keyNumber; i++) {
            target.keyValues[target.keyNumber++] = keyValues[i];
        }
    }
}